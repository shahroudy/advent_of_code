...
class SpaceStoichiometry:
    ...
    def process_rules(self):
        ...
        for line in self.lines:
            ...
            reaction = {"product_count": product_count, "elem": elems, "elem_count": elem_count}
    ...
    def needed_ORE_for_n_FUEL(self, n):
        ...
        while True:
            ...
            if not elems:
                break
            if not changed:
                elems_sorted = sorted(elems, key=lambda x: self.reactions[x]["order"], reverse=True)
                needed[elems_sorted[0]] = self.reactions[elems_sorted[0]]["product_count"]
            changed = False
            for elem in elems:
                ...
                if units > 0:
                    ...
                    for ne, nc in zip(reaction["elem"], reaction["elem_count"]):
                        needed[ne] += nc * units
        return needed["ORE"]
